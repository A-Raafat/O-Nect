...
def Calculate_D(PCM_Frames, TCM_x, TCM_y , Type='Radians'):
    ...
    for i in range(len(PCM_Frames)):  # Gathering the x, y coords together to compute the vectors in a convenient way
        ...
        d1_angle=np.dot(CC12, CC15)/(CC12_mag*CC15_mag)
        d2_angle=np.dot(C6C, C6C7)/(C6C_mag*C6C7_mag)
        if d1_angle >=1:
            d1_angle=1
        if d2_angle >=1:
            d2_angle=1
                       
        D1.append(np.arccos(d1_angle)) # My implimentation for the angle
        D2.append(np.arccos(d2_angle))   # My implimentation for the angle
...
def Add_Features_To_dataframe(file_name,temp, File, PCM_Frames, TCM_x, TCM_y, L, R, D1, D2, D3, name):
    
    keys=list(PCM_Frames['0'].keys())
    

    ################################# Creates columns ######################################
    if not 'Head & Neck_x_PCM' in File:
        File.insert(0, 'ID', file_name)
        for i in range(len(PCM_Frames)):
            File['ID'].iloc[i]=file_name+'_'+str(i)
...
################################ Add values ###########################################
        ...
        for i in range(len(PCM_Frames)):
            File.iloc[i,38:66]=list(PCM_Frames[str(i)].values())

        for i in range(len(L)):
            File.iloc[i,70:82]=L[i,2:]

        for i in range(len(R)):
            File.iloc[i,84:96]=R[i,2:] 
        ...
        for i in range(len(PCM_Frames)):
            ...
            temp['ID'].iloc[i]=file_name+'_'+str(i)
        ...
        temp['Head & Neck_L']=L[:,0]
        temp['Trunk_L']=L[:,1]

        for i in range(4,len(keys),2):
            temp[keys[i].replace('_x','')+'_L']=0


        temp['Head & Neck_R']=R[:,0]
        temp['Trunk_R']=R[:,1]

        for i in range(4,len(keys),2):
            temp[keys[i].replace('_x','')+'_R']=0
...
################################ Add values ###########################################
        ...
        for i in range(len(PCM_Frames)):
            temp.iloc[i,37:65]=list(PCM_Frames[str(i)].values())

        for i in range(len(L)):
            temp.iloc[i,69:81]=L[i,2:]

        for i in range(len(R)):
            temp.iloc[i,83:95]=R[i,2:] 
        
        return File.append(temp)
